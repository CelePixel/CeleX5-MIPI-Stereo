...
void DataProcessThreadEx::setCeleX(CeleX5* pCeleX5, int device_index)
{
	m_pCeleX5 = pCeleX5;
	m_iDeviceIndex = device_index;
	m_dataProcessor5.setDeviceIndex(device_index);
}

void DataProcessThreadEx::sePlayBackEnabled(bool state)
{
	m_bPlaybackBinFile = state;
	m_dataProcessor5.setPlaybackEnabled(state);
}
...
void DataProcessThreadEx::run()
{
	while (m_bRun)
	{
		//cout << "---------- DataProcessThread::run ----------: index = " << m_iDeviceIndex << endl;
#ifndef _WIN32
		...
		if (m_bPlaybackBinFile) //for playback
		{
			if (m_queueData.size() > 0)
			{
				...
				if (dataLen > 0)
				{
					//cout << "------------- pop: m_iDeviceIndex = " << m_iDeviceIndex << endl;
		...
		else //--- for real display ---
		{
			...
			m_pCeleX5->getMIPIData(m_vecMIPIPackage, time_stamp_end, imu_data, m_iDeviceIndex);
