...
std::time_t    g_ttBinFileStartTimestamp = 0;
...
CeleX5DataProcessor::CeleX5DataProcessor()
	...
	, m_iDeviceIndex(0)
	, m_bDataAligned(false)
	, m_ttBinFileStartTimestamp(0)
	, m_bPlaybackEnabled(false)
{
	m_vecEventData.reserve(1000000);
...
#ifdef _ENABLE_LOG_FILE_
	poll_t = clock();
	cout << "  Elapsed Time: " << (poll_t - begin_t) << " (ms)" << std::endl;
#endif
}
...
void CeleX5DataProcessor::setBinFileStartTimestamp(std::time_t timestamp)
{
	cout << __FUNCTION__ << ": m_ttBinFileStartTimestamp = " << timestamp << endl;
	m_ttBinFileStartTimestamp = timestamp;
}

void CeleX5DataProcessor::clearData()
{
	m_vecEventData.clear();
	m_vecEventData_ForUser.clear();
	m_vectorIMUData.clear();
	...
	m_bDataAligned = false;
	g_ttBinFileStartTimestamp = 0;
}

void CeleX5DataProcessor::setPlaybackEnabled(bool state)
{
	cout << "CeleX5DataProcessor::setPlaybackEnabled = " << state << endl;
	m_bPlaybackEnabled = state;
}
...
void CeleX5DataProcessor::processMIPIEventTimeStamp()
{
	...
	if (m_bPlaybackEnabled)
	{
		if (!m_bDataAligned)
		{
			if (m_ttBinFileStartTimestamp + m_uiEventTCounter * m_uiCurrentEventTUnit / 1000 >= g_ttBinFileStartTimestamp)
			{
				cout << "--------- m_uiEventTCounter = " << m_uiEventTCounter << ", index = " << m_iDeviceIndex << endl;
				m_uiEventTCounter = 0;
				m_uiEventTCounter_Total = 0;
				m_uiPackageTCounter = 0;
				m_bDataAligned = true;
