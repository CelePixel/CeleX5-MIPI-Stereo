	...
	, m_iDeviceIndex(0)
{
	...
	m_pFrameBuffer1_ForUser = new unsigned char[CELEX5_PIXELS_NUMBER];
	m_pFrameBuffer2_ForUser = new unsigned char[CELEX5_PIXELS_NUMBER];
	m_pFrameBuffer3_ForUser = new unsigned char[CELEX5_PIXELS_NUMBER];
	m_pFrameBuffer4_ForUser = new unsigned char[CELEX5_PIXELS_NUMBER];
	m_pFrameBuffer5_ForUser = new unsigned char[CELEX5_PIXELS_NUMBER];
	m_pFrameBuffer6_ForUser = new unsigned char[CELEX5_PIXELS_NUMBER];
...
{
	memcpy(buffer, m_pFrameBuffer1_ForUser, CELEX5_PIXELS_NUMBER);
}
...
void CeleX5DataProcessor::getEventPicBuffer( unsigned char* buffer, CeleX5::emEventPicType type)
{
	if (type == CeleX5::EventBinaryPic)
		...
		memcpy(buffer, m_pFrameBuffer2_ForUser, CELEX5_PIXELS_NUMBER);
	else if (type == CeleX5::EventAccumulatedPic)
		memcpy(buffer, m_pFrameBuffer3_ForUser, CELEX5_PIXELS_NUMBER);
	else if (type == CeleX5::EventSuperimposedPic)
		memcpy(buffer, m_pFrameBuffer4_ForUser, CELEX5_PIXELS_NUMBER);
	else if (type == CeleX5::EventDenoisedBinaryPic)
		memcpy(buffer, m_pFrameBuffer5_ForUser, CELEX5_PIXELS_NUMBER);
	else if (type == CeleX5::EventCountPic)
		memcpy(buffer, m_pFrameBuffer6_ForUser, CELEX5_PIXELS_NUMBER);
}
...
void CeleX5DataProcessor::parseEventDataFormat2(uint8_t* pData, int dataSize)
{
	...
	for (int i = 0; i < dataSize - 6; i += 7)
	{
		...
		{
			if (m_uiEventTCounter > m_uiEventTCountForRemove)
			{
...
void CeleX5DataProcessor::checkIfShowImage()
{
	...
	{
		if (m_uiEventRowCycleCount >= m_uiEventRowCycleCountForShow && m_uiRowCount >= 50 * m_uiEventRowCycleCountForShow)
		{
...
bool CeleX5DataProcessor::createImage()
{
	...
	if (m_emCurrentSensorMode == CeleX5::Full_Optical_Flow_S_Mode ||
		m_emCurrentSensorMode == CeleX5::Full_Optical_Flow_M_Mode)
	{
		memcpy(m_pFrameBuffer2_ForUser, pOpticalFlowPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer3_ForUser, pOpticalFlowSpeedPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer4_ForUser, pOpticalFlowDirectionPic, CELEX5_PIXELS_NUMBER);
	}
	else if (m_emCurrentSensorMode == CeleX5::Full_Picture_Mode)
	{
		memcpy(m_pFrameBuffer1_ForUser, pFullPic, CELEX5_PIXELS_NUMBER);
	}
	else if (m_emCurrentSensorMode == CeleX5::Event_Address_Only_Mode)
	{
		memcpy(m_pFrameBuffer1_ForUser, pEventBinaryPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer5_ForUser, pEventDenoisedBinaryPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer6_ForUser, pEventCountPic, CELEX5_PIXELS_NUMBER);
	}
	else if (m_emCurrentSensorMode == CeleX5::Event_Intensity_Mode)
	{
		memcpy(m_pFrameBuffer1_ForUser, pEventBinaryPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer2_ForUser, pEventGrayPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer3_ForUser, pEventAccumulatedPic, CELEX5_PIXELS_NUMBER);
		memcpy(m_pFrameBuffer4_ForUser, pEventSuperimposedPic, CELEX5_PIXELS_NUMBER);
...
void CeleX5DataProcessor::generateFPN(std::string filePath, int device_index)
{
	m_iGeneratingFPNIndex = device_index;
...
void CeleX5DataProcessor::setDeviceIndex(int device_index)
{
	cout << __FUNCTION__ << ": device_index = " << device_index << endl;
	m_iDeviceIndex = device_index;
	m_pCX5ProcessedData->setDeviceIndex(device_index);
}
