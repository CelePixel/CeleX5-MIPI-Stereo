...
CeleX5::~CeleX5()
{
	...
	if (m_pCeleDriver)
	{
		...
		for (int i = 0; i < MAX_SENSOR_NUM; i++)
		{
			m_pCeleDriver->clearData(i);
			m_pCeleDriver->closeStream(i);
			m_pCeleDriver->closeUSB(i);
		}
...
bool CeleX5::openSensor(DeviceType type)
{
	...
	if (CeleX5::CeleX5_MIPI == type)
	{
		...
		for (int i = 0; i < m_pCeleDriver->getDeviceCount(); i++) //just for two sensors
		{
			...
			std::string serialNumber = getSerialNumber(i);
			m_pSequenceMgr->clearData();
			m_mapCfgModified.clear();
			m_mapCfgDefaults.clear();
...
{
	//cout << "getMIPIData: index = " << device_index << ", data size = " << buffer.size() << endl;
...
void CeleX5::disableFrameModule(int device_index)
{
	m_pDataProcessThread[device_index]->getDataProcessor5()->disableFrameModule();
}

void CeleX5::enableFrameModule(int device_index)
{
	m_pDataProcessThread[device_index]->getDataProcessor5()->enableFrameModule();
}

bool CeleX5::isFrameModuleEnabled(int device_index)
{
	return m_pDataProcessThread[device_index]->getDataProcessor5()->isFrameModuleEnabled();
}

void CeleX5::disableIMUModule(int device_index)
{
	m_pDataProcessThread[device_index]->getDataProcessor5()->disableIMUModule();
}

void CeleX5::enableIMUModule(int device_index)
{
	m_pDataProcessThread[device_index]->getDataProcessor5()->enableIMUModule();
}

bool CeleX5::isIMUModuleEnabled(int device_index)
{
	return m_pDataProcessThread[device_index]->getDataProcessor5()->isIMUModuleEnabled();
}
...
bool CeleX5::getEventDataVector(std::vector<EventData> &vecData, int device_index)
{
	return m_pDataProcessThread[device_index]->getDataProcessor5()->getEventDataVector(vecData);
}
bool CeleX5::getEventDataVector(std::vector<EventData> &vecData, uint64_t& frameNo, int device_index)
{
	return m_pDataProcessThread[device_index]->getDataProcessor5()->getEventDataVector(vecData, frameNo);
}
bool CeleX5::getEventDataVectorEx(std::vector<EventData> &vecData, std::time_t& time_stamp, int device_index, bool bDenoised)
{
	return m_pDataProcessThread[device_index]->getDataProcessor5()->getEventDataVectorEx(vecData, time_stamp, bDenoised);
}
...
void CeleX5::setSensorFixedMode(CeleX5Mode mode, int device_index)
{
	...
	m_emSensorFixedMode[device_index] = mode;
}
...
void CeleX5::setClockRate(uint32_t value, int device_index)
{
	...
	if (CeleX5::CeleX5_MIPI == m_emDeviceType)
	{
		...
		{
			if (value == 70)
				writeRegister(159, -1, -1, 24, device_index);
			else if (value = 100)
				writeRegister(159, -1, -1, 12, device_index);
			else
...
void CeleX5::stopSensor()
{
	for (int i = 0; i < MAX_SENSOR_NUM; i++)
	{
		//hard_reset: bit[3:2], als_enable: bit[1] (1=disable, 0=enable)
		m_pCeleDriver->i2c_set(254, 10, i); //b'1010
	}
}
...
bool CeleX5::openBinFile(std::string filePath, int device_index)
{
	...
	m_pDataProcessThread[device_index]->sePlayBackEnabled(true);
...
bool CeleX5::readBinFileData(int device_index)
{
	...
	if (dataLen > 0)
	{
		...
		if ((0x02 & m_stBinFileHeader[device_index].data_type) == 0x02) //has IMU data
		{
			...
			//cout << "device_index = " << device_index << ", timeStamp = " << timeStamp << endl;
...
void CeleX5::alignBinFileData()
{
	for (int device_index = 0; device_index < MAX_SENSOR_NUM; device_index++)
	{
		if (!m_ifstreamPlayback[device_index].is_open())
		{
			return;
		...
		if (dataLen > 0)
		{
			...
			if ((0x02 & m_stBinFileHeader[device_index].data_type) == 0x02) //has IMU data
			{
				...
				m_ifstreamPlayback[device_index].read((char*)&timeStamp, 8);
				//cout << "device_index = " << device_index << ", timeStamp = " << timeStamp << endl;
				m_pDataProcessThread[device_index]->getDataProcessor5()->setBinFileStartTimestamp(timeStamp);

				if (timeStamp > g_ttBinFileStartTimestamp)
					g_ttBinFileStartTimestamp = timeStamp;

				cout << "g_ttBinFileStartTimestamp = " << g_ttBinFileStartTimestamp << endl;
...
void CeleX5::replay()
{
	...
	for (int i = 0; i < MAX_SENSOR_NUM; i++)
	{
		...
		m_pDataProcessThread[i]->clearData();
	}
}
...
void CeleX5::setPlaybackEnabled(bool state)
{
	for (int i = 0; i < MAX_SENSOR_NUM; i++)
	{
		m_pDataProcessThread[i]->sePlayBackEnabled(state);
		if (!state) //stop playback
		{
			...
			m_pDataProcessThread[i]->getDataProcessor5()->setSensorFixedMode(m_emSensorFixedMode[i]);
			if (m_emSensorFixedMode[i] == CeleX5::Event_Address_Only_Mode)
				setEventDataFormat(2, i);
			else
				setEventDataFormat(1, i);
		}
	}
	if (state)
	{
		m_pCeleDriver->pauseSaveSensorData();
	}
	else
	{
		m_pCeleDriver->restartSaveSensorData();
	}
}
...
bool CeleX5::configureSettings(CeleX5::DeviceType type, int device_index)
{
	if (CeleX5::CeleX5_MIPI == type)
	{
		...
		if (device_index == 1)
		{
			wireIn(159, 12, 0xFF, device_index);
		}
		//Enable PLL
