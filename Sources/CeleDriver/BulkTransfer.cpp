...
void generate_image(uint8_t *buffer, int length)
{
	...
	if (current_package_m == nullptr)
	{
		...
		{
			if (write_frame_index_m == read_frame_index_m)
			{
...
void generate_image1(uint8_t *buffer, int length)
{
	...
	if (current_package_s == nullptr)
	{
		...
		{
			if (write_frame_index_s == read_frame_index_s)
			{
			...
			printf("------- generate_image1: buffer is full! -------\n");
		}
		//printf("--- package_count = %d, write_frame_index = %d\n", package_count, write_frame_index);
	}
...
bool GetPicture(vector<uint8_t> &Image, int device_index)
{
	if (bRunning_m == true)
	{
		if (0 == device_index)
		{
			...
			{
				if (read_frame_index_m != write_frame_index_m)
				{
		...
		else
		{
			...
			{
				if (read_frame_index_s != write_frame_index_s)
				{
...
void PauseSaveSensorData()
{
	ClearData();
	g_bSaveSensorData = false;
}

void RestartSaveSensorData()
{
	ClearData();
	g_bSaveSensorData = true;
}
